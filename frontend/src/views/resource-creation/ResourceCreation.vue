...
<template>
  <main class="resource-creation-page">
    ...
    <template v-else-if="currentMode === 'generate'">
      <nav class="creation-flow" aria-label="资源生成流程"><button :class="{ active: currentGenerateStep === 1, completed: currentGenerateStep > 1 }" @click="navigateGenerateStep(1)">{{ currentGenerateStep > 1 ? '✓' : '●' }} 选择资源</button><i>→</i><button :disabled="!selectedResources.length" :class="{ active: currentGenerateStep === 2, completed: currentGenerateStep > 2 }" @click="navigateGenerateStep(2)">{{ currentGenerateStep > 2 ? '✓' : currentGenerateStep === 2 ? '●' : '○' }} 生成设置</button><i>→</i><button :disabled="!hasGeneratedResources" :class="{ active: currentGenerateStep === 3 }" @click="navigateGenerateStep(3)">{{ currentGenerateStep === 3 ? '●' : '○' }} AI共创</button></nav>
